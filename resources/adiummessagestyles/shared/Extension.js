...
function appendCustumMessage(html,type,index,past) {
	var id = Math.round(Math.random()*10000)+'_'+Math.round(Math.random()*10000), getSib = function(which,el){
		el = el[which+'Sibling'];
		...
		messages[ind] = nid;
		if (ins) {
			add && ins.parentNode.replaceChild(sib,ins);
			sib && ins.parentNode.removeChild(sib);
		}
		curins.id = 'insert';
