																									 ...
																									 pjmedia_sdp_media **p_m)
{
	...
	for (i=0; i<cnt; ++i)
	{
		...
		if ((codec_info[i].packings & PJMEDIA_VID_PACKING_PACKETS) == 0 /*|| codec_info[i].dir != PJMEDIA_DIR_ENCODING_DECODING*/) // ононб
		{
			continue;
		}
		...
		/* Find maximum bitrate in this media */
		vfd = pjmedia_format_get_video_format_detail(&codec_param.enc_fmt, PJ_TRUE);
																						 ...
																						 pjmedia_sdp_session **p_sdp )
{
...
#if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0)
	{
		...
		{
			status = pjmedia_endpt_create_video_sdp(endpt, pool, &sock_info[i], 0, &m);
			if (status != PJ_SUCCESS)
				return status;
			sdp->media[sdp->media_count++] = m;
		}
