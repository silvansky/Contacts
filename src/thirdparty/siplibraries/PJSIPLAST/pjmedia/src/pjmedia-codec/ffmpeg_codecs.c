																							...
																							pj_pool_factory *pf)
{
	...
	avcodec_init(); // ÏÎÏÎÂ: Óñòàðåâøèé âûçîâ ôóíêöèè. Âñå ñàìî ïðîèñõîäèò â avcodec_register_all()
...
#   define AVMEDIA_TYPE_VIDEO	CODEC_TYPE_VIDEO
		...
		*/
		...
		//if (( (c->encode || c->encode2) && (desc->info.dir & PJMEDIA_DIR_ENCODING)) || (c->decode && (desc->info.dir & PJMEDIA_DIR_DECODING))) // ÏÎÏÎÂ: äëÿ x264 èñïîëüçóåòñÿ c->encode2
		{
			continue;
		}
		...
		//if (c->pix_fmts && (c->encode || c->encode2)) // ÏÎÏÎÂ: äëÿ x264 èñïîëüçóåòñÿ c->encode2
		{
		...
		/* Get ffmpeg encoder instance */
		if (c->encode && !desc->enc)
		//if ((c->encode || c->encode2) && !desc->enc) // ÏÎÏÎÂ: äëÿ x264 èñïîëüçóåòñÿ c->encode2
		{
			desc->info.dir |= PJMEDIA_DIR_ENCODING;
			desc->enc = c;
		}
																		 ...
																		 pj_mutex_t *ff_mutex)
{
	...
	//if (ff->param.dir & PJMEDIA_DIR_ENCODING) // ÏÎÏÎÂ
	if (ff->enc && ff->param.dir & PJMEDIA_DIR_ENCODING)
	{
		AVCodecContext *ctx = ff->enc_ctx;
		...
		*/
#if LIBAVCODEC_VER_AT_LEAST(52,113)  // ÏÎÏÎÂ: óáðàë ïîòîìó êàê â íîâîì FFMpeg ýòîãî íåò
		ctx->rc_lookahead = 0;
#endif
	}
