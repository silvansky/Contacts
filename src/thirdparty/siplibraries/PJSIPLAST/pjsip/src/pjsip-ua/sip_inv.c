...
static void inv_on_state_confirmed( pjsip_inv_session *inv, pjsip_event *e)
{
	...
	else if (tsx->method.id == PJSIP_INVITE_METHOD && tsx->role == PJSIP_ROLE_UAS)
	{
		/*
		...
		*/
		if (tsx->state == PJSIP_TSX_STATE_TRYING)
		{
			...
			if (inv->invite_tsx && inv->invite_tsx != tsx)
			{
				...
				status = pjsip_dlg_create_response( inv->dlg, rdata, code, &reason, &tdata);
				if (status != PJ_SUCCESS)
					return;
			...
			if (status != PJ_SUCCESS)
			{
				status = pjsip_dlg_create_response(inv->dlg, rdata, st_code, NULL, &tdata);
				if (status != PJ_SUCCESS)
					return;
		...
		else if (tsx->state == PJSIP_TSX_STATE_CONFIRMED)
		{
			...
			* the INVITE request.
			*/
			if (tsx->status_code/100 == 2 && e->body.tsx_state.type == PJSIP_EVENT_RX_MSG) 
			{
				inv_check_sdp_in_incoming_msg(inv, tsx,
					e->body.tsx_state.src.rdata);
	...
	else if (tsx->method.id == PJSIP_INVITE_METHOD && tsx->role == PJSIP_ROLE_UAC)
	{
		...
		*/
		...
		else if (tsx->state == PJSIP_TSX_STATE_TERMINATED && tsx->status_code/100 == 2) 
		{
			pj_status_t status;
