/* $Id: pjsua_media.c 3945 2012-01-27 09:12:59Z nanang $ */
/* 
																					 ...
																					 int *sip_err_code)
{
	...
	*/
	...
	status = pjmedia_endpt_create_base_sdp(pjsua_var.med_endpt, pool, NULL, &origin, &sdp);
	if (status != PJ_SUCCESS)
		return status;
	...
	for (mi=0; mi<call->med_cnt; ++mi)
	{
			...
			*/
			...
			else
			{
				...
				default:
					...
					*/
					pj_assert(call->inv && call->inv->neg && sdp_neg_state == PJMEDIA_SDP_NEG_STATE_DONE);
					{
		...
		/* Give to transport */
		status = pjmedia_transport_encode_sdp(call_med->tp, pool, sdp, rem_sdp, mi);
		if (status != PJ_SUCCESS)
		{
	...
	if (pjsua_var.ua_cfg.nat_type_in_sdp)
	{
		...
		{
			value.slen = pj_ansi_snprintf(nat_info, sizeof(nat_info), "%d", pjsua_var.nat_type);
		}
		else
		{
...
PJ_DEF(pj_status_t) pjsua_set_null_snd_dev(void)
{
	...
	* a null port.
	*/
	status = pjmedia_master_port_create(pjsua_var.snd_pool, pjsua_var.null_port, conf_port, 0, &pjsua_var.null_snd);
	if (status != PJ_SUCCESS)
	{
